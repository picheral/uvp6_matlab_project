...
for j = 2 : nb_zones + 1
    ...
    aa = find(y_ecart_area == min(y_ecart_area));
    if ~isempty(aa)
        if aa(1) > 1 && aa(end) < numel(x)
            mat_cor_ecart_area(j-1) = x(aa(1));
        end
    end
    
    aa = find(y_ecart_grey == min(y_ecart_grey));
    if ~isempty(aa)
        if aa(1) > 1 && aa(end) < numel(x)
            mat_cor_ecart_grey(j-1) = x(aa(1));
        end
    end
    
    aa = find(y_ecart_fit_a == min(y_ecart_fit_a));
    %         aa = find(y_ecart_fit_a <= 0);
    if ~isempty(aa)
        if aa(1) > 1 && aa(end) < numel(x)
            mat_cor_fit_a(j-1) = x(aa(1));
        end
    end
    
    aa = find(y_ecart_fit_b == min(y_ecart_fit_b));
    if ~isempty(aa)
        if aa(1) > 1 && aa(end) < numel(x)
            mat_cor_fit_b(j-1) = x(aa(1));
        end
    end
    ...
    if round(j/5)-1 == j/5 - 1
        ...
        orient tall
        set(gcf,'PaperPositionMode','auto')
        print(gcf,'-dpng',[results_folder,'matrice_',num2str(nb_zones),'_zones_3_2_',num2str(mat_adj(1)),'_zone_',num2str(j),'_',filename]);
...
imagesc(ff,[8 30]);
xlabel(['cor area POLY 3 [',num2str(min(min(image_cor_area))),'  -  ',num2str(max(max(image_cor_area))),']']);
title(titre);
colorbar
...
imagesc(ff,[8 30]);
xlabel(['cor grey POLY 3 [',num2str(min(min(image_cor_grey))),'  -  ',num2str(max(max(image_cor_grey))),']']);
...
imagesc(ff,[8 30]);
xlabel(['cor ecarts area POLY 2 [',num2str(min(min(image_cor_ecarts_area))),'  -  ',num2str(max(max(image_cor_ecarts_area))),']']);
colorbar
...
imagesc(ff,[8 30]);
xlabel(['cor ecarts grey POLY 2 [',num2str(min(min(image_cor_ecarts_grey))),'  -  ',num2str(max(max(image_cor_ecarts_grey))),']']);
colorbar
...
imagesc(ff,[8 30]);
xlabel(['cor fit a POLY 2 [',num2str(min(min(image_cor_fit_a))),'  -  ',num2str(max(max(image_cor_fit_a))),']']);
colorbar
...
imagesc(ff,[8 30]);
xlabel(['cor fit b POLY 2 [',num2str(min(min(image_cor_fit_b))),'  -  ',num2str(max(max(image_cor_fit_b))),']']);
colorbar

% ------------------ Sauvegarde image --------------------------
orient tall
set(gcf,'PaperPositionMode','auto')
print(gcf,'-dpng',[results_folder,'matrice_',num2str(nb_zones),'_zones_3_2_',num2str(mat_adj(1)),'_post_raw_',filename]);
...
ff = imresize(ff,[128.5 154],'bicubic');
ff = exp(ff*0.1155);
...
maxi = max(max(ff));
imagesc(ff,[1 6]);
xlabel(['cor area POLY 3 gauss ',num2str(gauss),' [x',num2str(maxi,2),']']);
title(titre);
colorbar
...
maxi = max(max(ff));
imagesc(ff,[1 6]);
xlabel(['cor grey POLY 3 gauss ',num2str(gauss),' [x',num2str(maxi,2),']']);
title([fit_type,'  sym AFTER ref : ',char(gain_ref)]);
colorbar
if isempty(find(isnan(ff), 1))
...
maxi = max(max(ff));
imagesc(ff,[1 6]);
xlabel(['cor ecarts area POLY 2 gauss ',num2str(gauss),' [x',num2str(maxi,2),']']);
colorbar
if isempty(find(isnan(ff), 1))
    file = [filename,'_ecarts_area_gauss_',num2str(gauss),'.mat'];
    eval(['save ',file,' ff -ascii']);
end
...
maxi = max(max(ff));
imagesc(ff,[1 6]);
xlabel(['cor ecarts grey POLY 2 gauss ',num2str(gauss),' [x',num2str(maxi,2),']']);
colorbar
if isempty(find(isnan(ff), 1))
...
maxi = max(max(ff));
imagesc(ff,[1 6]);
xlabel(['cor fit a POLY 2 gauss ',num2str(gauss),' [x',num2str(maxi,2),']']);
colorbar
if isempty(find(isnan(ff), 1))
...
maxi = max(max(ff));
imagesc(ff,[1 6]);
xlabel(['cor fit b POLY 2 gauss ',num2str(gauss),' [x',num2str(maxi,2),']']);
colorbar
if isempty(find(isnan(ff), 1))
...
orient tall
set(gcf,'PaperPositionMode','auto')
print(gcf,'-dpng',[results_folder,'matrice_',num2str(nb_zones),'_zones_3_2_',num2str(mat_adj(1)),'_post_res_',filename]);
