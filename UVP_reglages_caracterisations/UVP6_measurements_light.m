...
function [max_h_profile_i,angle_deg,thick_left,thick_right,mean_left,mean_right,Intensity] = UVP6_measurements_light(img,segmentation,angle_limit,pixel,figure_plot)
...
thick_left = pixel * (limits(4,2) - limits(4,1));
thick_right = pixel * (limits(end-4,2) - limits(end-4,1));

% Intensity measurements
mean_left = Intensity(3);
mean_right = Intensity(end-3);
...
if figure_plot == 1
    ...
    plot([1:size(img_crop_mean_norm,2)]*pixel,img_crop_mean_norm(4,:),'r-')
    hold on
    plot([1+shift:size(img_crop_mean_norm,2)+shift]*pixel,img_crop_mean_norm(end-4,:),'g-')
    legend('right','left');
    titre = ['NORMALIZED INTENSITY'];
    title(titre,'fontsize',10);
    xlabel('THICKNESS [mm]','fontsize',12);
    ylabel('INTENSITY (normalized)','fontsize',12);
    axis([ round((limits(3,2) + limits(3,1))/2*pixel) - 15 round((limits(4,2) + limits(4,1))/2*pixel) + 15 0 300])
