% ----------------------------------------------------------------------- %
% --------------------- REGLAGE VERRINE VERTICAL ------------------------ %
% ----------------------------------------------------------------------- %
%
% modified by Camille Catalano
...
%Set the best angle and best ET boxes
...
while fin == hh | last_image <= 10
    ...
    if ~isempty(strfind(base_grey(end).name,'.png'))
        if base_grey(end).datenum > last_date
            ...
            % Angle warning if the beam is not close to the horizontal
            ...
            %% loop on the vertical profiles
            %if loop that goes through the beam's lenghts and each step
            %integrates a 21 pixels wide section spaced by step pixels which
            %step is calculated at the beginnng of the code
            for i = 1 : N
                %% Left part of the beam
                %each section is the mean of a 21 pixels wide section
                img_crop_mean_orig(N+1-i,:) = mean(img(mid-step*i-10:mid-step*i+10,700:end-700));
                intervalles(N+1-i,:) = [mid-step*i-10:mid-step*i+10];
            ...
            %% update of the table
            %Update of the matrixs in which the interesting parameters are
            %stored
            ...
            diff_peak_angle = atand((diff_peak_poly(N+1+floor(N/2))-diff_peak_poly(N+1))/(floor(N/2)*step));
            diff_max_angle = atand((diff_max_poly(N+1+floor(N/2))-diff_max_poly(N+1))/(floor(N/2)*step));
            ...
            hold on
            plot([mid-step*N-100 mid+step*N+100]*pixel,[0,0],'r')
            plot6 = plot(intervalles([1:2*N+1],11)*pixel,diff_peak_poly,'k');
            title({'fig 1 - Intensity difference between upper and lower part of ','the beam divided by the total intensity ZOOM'})
            ...
            title({'fig3 - Difference between max of the bottom','and top peaks divided by the mean'})
            xlabel('Lenght of the beam (mm)')
            ylabel('Normalized intensity')
            axis([0 2464*pixel -0.8 0.8])
            text2 = text(20,-0.7,['slope :' num2str(1000*poly2(1),'%.3g')]);
            ...
            title({'fig 2 - Intensity difference between upper and lower part of ','the beam divided by the total intensity'})
            xlabel('Lenght of the beam (mm)')
            ylabel('Normalized intensity')
...
if cc == 2
    %% Ask metadata
    UVP_cam = input('Enter CAMERA serial number (xxx) ','s');
...
%     if isempty(suppl); suppl = ''; end
    ...
    for i = 1:N
        plot([mid+step*i mid+step*i],[150 size(img_rot,1)-150],'g-')
        plot([mid-step*i mid-step*i],[150 size(img_rot,1)-150],'g-')
    end
    ...
    cd(path);
    mkdir([datestr(now,10) datestr(now,5) datestr(now,7) '_Reglage_verrines_ve_' UVP_light]);
    cd([datestr(now,10) datestr(now,5) datestr(now,7) '_Reglage_verrines_ve_' UVP_light]);
    
    fid_uvp = fopen(fullfile(cd,[datestr(now,10) datestr(now,5) datestr(now,7) '_Reglage_verrines_ve_' UVP_light '.txt']),'w');
    ...
    fprintf(fid_uvp,'%s\r',['UVP CAMERA s/n             : ',num2str(UVP_cam)]);
    fprintf(fid_uvp,'%s\r',['UVP LIGHT s/n              : ',num2str(UVP_light)]);
    ...
    fprintf(fid_uvp,'%s\r',['Max Intensity          : ',num2str(round(mean(max_int(end-9:end)),2))]);
    fprintf(fid_uvp,'%s\r',['Mean Intensity         : ',num2str(round(mean(mean_int(end-9:end)),2))]);
    fprintf(fid_uvp,'%s\r',['Total Intensity        : ',num2str(int_tot)]);
    fprintf(fid_uvp,'%s\r',['Angle of Intensity diffrence between upper and lower part of the beam       : ',num2str((mean(poly_slope(end-9:end))))]);
    fprintf(fid_uvp,'%s\r',['Standard deviation of Intensity difference between upper and lower part of the beam : ',num2str(round(mean(ecart1(end-9:end))*10,3))]);
...
%     fprintf(fid_uvp,'%s\r',['REFERENCE MEAN INTENSITY : ',num2str(ref_int)]);
    ...
    %% save full figure
    saveas(f,fullfile(cd,[datestr(now,10) datestr(now,5) datestr(now,7) '_Reglage_verrives_' UVP_light '_graphs.png']))
    fclose(fid_uvp);
    close(f)
    
    %% save figure of max intensity vs time
    fig = figure;
    ...
    saveas(fig,fullfile(cd,[datestr(now,10) datestr(now,5) datestr(now,7) '_Reglage_verrives_' char(UVP_light) '_int_evolution.png']));
