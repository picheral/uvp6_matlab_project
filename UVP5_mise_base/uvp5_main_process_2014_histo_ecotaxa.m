...
function [base ] = uvp5_main_process_2014_histo_ecotaxa(base,skip_histo,fichier,recpx,uvp5_cor_mat,pasvert,results_dir,save_histo,ligne,depth_offset,groupe,calibration,process_calib )
...
elseif process_histo == 1;
    ...
    if (exist(tta) == 2 && (exist(ttb) == 2 ||exist(ttc) == 2)==1 );
        ...
        liste = [ Image(kk(1):gg(end)) Pressure(kk(1):gg(end))/10 Flag(kk(1):gg(end))];
        ...
        % ----------- Cas ou la pression ne varie pas (bassin) ---------------
%         zmin = min(liste(:,2));
%         if zmax-zmin < 5 
%             listecor = liste;
%         else
...
%         end
        ...
        for z=pasvert:pasvert:profondeur(end)
            ...
            % Recherche entre listea et listeb / imglist est la liste des index des lignes du bloc
            ...
            if (isempty(imglist) | (isempty(imgint)));
                ...
                for img=1:nbimgy;
                    ...
                    else    % L'image n'est pas traitée car pas descente
                        if isempty(imgbru)
